...
#-----below here is unique to logistic regression------ 

#Cs is the values of C that need to be evaluated for parameter optimization
...
for C in Cs:
    ...
    scores = cross_val_score(lr, protein_data, combind_labels, cv = cv, scoring = SCORING_METHOD)
...
ax = sns.barplot(x="C", y="score", data=df).set_title('LR - Regularization Optimization (C)')

#save the plot
ax.figure.savefig("lr-C-optimization-plot.png")
