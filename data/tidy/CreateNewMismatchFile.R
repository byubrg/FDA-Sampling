...
msiMatched = filter(full_join(msi, labels, by = 'sample'), mismatch == 0)
...
proteinMatched = filter(full_join(protein, labels, by = 'sample'), mismatch = 0)
...
proteinMismatched = mutate(proteinMismatched, sample = past0(sample, '5'))
